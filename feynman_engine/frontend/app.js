...
const histMinPt = document.getElementById("hist-minpt");
...
async function computeHistogram() {
  ...
  // Per-particle pT cut — required for IR-divergent γγ / jj.  0 means
  // use the backend's auto-default (25 for γγ, 30 for jj).
  if (histMinPt && parseFloat(histMinPt.value) > 0) {
    params.set("min_pT", histMinPt.value);
  }
